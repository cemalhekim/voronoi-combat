...
function voronoi(D::Delaunay)::Vector{Tuple{Point2f, Vector{Point2f}}}

	if isempty(D.triangles)
		return Vector{Vector{Point2f}}()
	end

	# Extract unique non-super sites
	all_sites = Set{Point2f}()
	for tri in D.triangles
		for p in (tri.edge.origin, tri.edge.next.origin, tri.edge.prev.origin)
			if p ∉ D.super_vertices
				push!(all_sites, Point2f(p.x, p.y))
			end
		end
	end
	sites = collect(all_sites)

	# Compute bounding box
	xs = getindex.(sites, 1)
	ys = getindex.(sites, 2)
	padding = 1.0
	bounds = (minimum(xs) - padding, minimum(ys) - padding, maximum(xs) + padding, maximum(ys) + padding)

	# Use original clipping function
	...
	site_cells = Vector{Tuple{Point2f, Vector{Point2f}}}()
	for P in sites
		cell = [Point2f(bounds[1], bounds[2]), Point2f(bounds[3], bounds[2]),
				Point2f(bounds[3], bounds[4]), Point2f(bounds[1], bounds[4])]
		for Q in sites
			if Q != P
				cell = clip_by_bisector(cell, P, Q)
			end
		end
		push!(site_cells, (P, cell))
	end
	return [cell for (_, cell) in site_cells]
end
...
function start_game()
	...
	on(ax.scene.events.mousebutton) do ev
		if ev.button==Mouse.left && ev.action==Mouse.press
			...
			sites = vcat(pos_player_1, pos_player_2)
			zipped = voronoi(delaunay_)
			cells = [cell for (_, cell) in zipped]

			for (site, cell) in zipped
				color = site in pos_player_1 ? :red : :blue
				push!(current_plot[], poly!(ax, cell; color=color, transparency=true, alpha=0.3))
			end
			
			if length(sites) != length(cells)
				@warn "Mismatch: sites and Voronoi cells count doesn't match."
				return
			end

			if length(sites) > 1
				area1 = sum(polygon_area, [cell for (site, cell) in zipped if site in pos_player_1])
				area2 = sum(polygon_area, [cell for (site, cell) in zipped if site in pos_player_2])
			else
				area1 = 4.0
				area2 = 0.0
			end
